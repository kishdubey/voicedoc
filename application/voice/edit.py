...
def trim(audio_path, time_start, time_finish):
    ...
    trim = (
        audio[: _to_milliseconds(time_start)] + audio[_to_milliseconds(time_finish) :]
    )
...
def concatenate(audio_path, audio_to_add_path, timestamp):
    ...
    concat = (
        audio[: _to_milliseconds(timestamp)]
        + audio_to_add
        + audio[_to_milliseconds(timestamp) :]
    )
...
def slice(audio, duration, before=True):
    ...
    """
    return (
        audio[: _to_milliseconds(duration)]
        if before
        else audio[_to_milliseconds(duration) :]
    )
